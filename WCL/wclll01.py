...
ACCESS_TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJkaGFuIiwicGFydG5lcklkIjoiIiwiZXhwIjoxNzU4MTYzODU5LCJpYXQiOjE3NTgwNzc0NTksInRva2VuQ29uc3VtZXJUeXBlIjoiU0VMRiIsIndlYmhvb2tVcmwiOiIiLCJkaGFuQ2xpZW50SWQiOiIxMTAwOTk2ODE5In0.mV3YSH2ryJyqsyQ5QHfhT5aGl1rwZez_RTYaO1hGViq8d_O7cArYS2kh1zZE_hPgRtoBRMlMM_xP1TOVGQPJrg"
...
def get_current_expiry_date():
    ...
    next_thursday = get_next_thursday(now.date())
...
def apply_colab_optimizations():
    """Apply specific optimizations for Google Colab"""
    if is_google_colab():
        ...
        # You can add more Colab-specific optimizations here
...
def fetch_and_check_conditions_improved():
    ...
    try:
        ...
        if (intraday_data_response and
            ...
            if C1 and C2 and C3 and C4 and C6 and C7 and C8 and C9 and C10 and C11 and not position_active:
                if not is_trading_allowed():
...
def run_trading_strategy_improved():
    ...
    # Apply Colab optimizations first
    apply_colab_optimizations()
    ...
    if not wait_for_market_feed_initialization(timeout_seconds=90):  # Longer timeout for Colab
